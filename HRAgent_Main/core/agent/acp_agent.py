...
The Agent Client Protocol (ACP) lets HRAgents power conversations using
...
the ACPAgent relays user messages and collects the response. HRAgents
...
from core.agent.acp_models import ACPModelInfo
...
from models.llm import LLM, ImageContent, Message, MessageToolCall, TextContent
from runtime.telemetry.logger import get_logger
from mcp_integration.config import MCPServer
from runtime.telemetry.observability.laminar import maybe_init_laminar, observe
from configuration.settings.acp_providers import (
    ACPFileSecretSpec,
...
from tools import Tool  # noqa: TC002
from tools.builtins.finish import FinishAction, FinishObservation
from utilities import maybe_truncate
from utilities.pydantic_secrets import (
    serialize_secret,
    validate_secret,
)
from utilities.redact import redact_text_secrets
...
def _mcp_config_to_acp_servers(
    ...
    """Translate HRAgents MCP servers into ACP MCP server objects.
...
class _HRAgentsACPBridge:
    """Bridge between HRAgents and ACP that accumulates session updates.
...
class ACPAgent(AgentBase):
    ...
    _client: Any = PrivateAttr(default=None)  # _HRAgentsACPBridge
    ...
    def supports_HRAgent_mcp(self) -> bool:
        """``False`` — HRAgents does not create in-process MCP tools here.
    ...
    def supports_runtime_model_switch(self) -> bool:
        ...
        See
        :meth:`~core.conversation.impl.local_conversation.LocalConversation.switch_acp_model`.
        """
    ...
    def init_state(
        ...
        from utilities.async_executor import AsyncExecutor
    ...
    def _acp_file_secret_dir(self, state: ConversationState, subdir: str) -> Path:
        ...
        if state.persistence_dir:
            ...
            root = Path(state.workspace.working_dir) / ".HRAgent" / "acp" / subdir
    ...
    def _reset_client_for_turn(
        ...
        ``_HRAgentsACPBridge.session_update`` as tool-call notifications
    ...
    def _finalize_successful_turn(
        ...
        # _HRAgentsACPBridge.session_update: one early ``started`` event per
