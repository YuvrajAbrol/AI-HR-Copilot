...
from core.execution.hooks.types import HookEventType
...
class HookConfig(BaseModel):
    ...
    Hooks can be configured either by loading from `.HRAgent/hooks.json` or
    by directly instantiating with typed fields:
        ...
        # Load from JSON file:
        config = HookConfig.load(".HRAgent/hooks.json")
    """
    ...
    def load(
        ...
        """Load config from path or search .HRAgent/hooks.json locations.

        Args:
            path: Explicit path to hooks.json file. If provided, working_dir is ignored.
            working_dir: Project directory for discovering .HRAgent/hooks.json.
        ...
        if path is None:
            ...
            search_paths = [
                base_dir / ".HRAgent" / "hooks.json",
                Path.home() / ".HRAgent" / "hooks.json",
            ]
