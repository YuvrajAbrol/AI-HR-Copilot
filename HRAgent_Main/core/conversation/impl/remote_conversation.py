...
from core.conversation.base import BaseConversation, ConversationStateProtocol
...
from core.conversation.exceptions import (
...
from tools.client_tool import ClientTool, ClientToolSpec
from utilities.redact import http_error_log_content
from core.workspace import LocalWorkspace, RemoteWorkspace
...
class RemoteConversation(BaseConversation):
    ...
    def __init__(
        ...
        **_: object,
        ...
        if should_create:
            ...
            from subagents.registry import get_registered_agent_definitions
            from tools.registry import get_tool_module_qualnames
        ...
        ws_timeout = float(os.getenv("HRAgentS_REMOTE_WS_READY_TIMEOUT", "30"))
        if not self._ws_client.wait_until_ready(timeout=ws_timeout):
            if os.getenv("HRAgentS_REMOTE_WS_READY_REQUIRED", "true").lower() in (
                "0",
                "false",
                "no",
            ):
                logger.warning(
                    ...
                    "reconciliation because HRAgentS_REMOTE_WS_READY_REQUIRED "
                    "is false.",
                    ws_timeout,
                    self._id,
                )
    ...
    def update_secrets(self, secrets: Mapping[str, SecretValue]) -> None:
        from security.validation.secret.secrets import SecretSource
