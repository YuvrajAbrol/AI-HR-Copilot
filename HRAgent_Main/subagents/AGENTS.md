...
This package (`HRAgent.sdk.subagent`) centralizes **subagent discovery** and **registration**.
...
3. **Project** file-based agents
   - `{project}/.agents/agents/*.md` then `{project}/.HRAgent/agents/*.md`
4. **User** file-based agents
   - `~/.agents/agents/*.md` then `~/.HRAgent/agents/*.md`
...
1. **Within a level** (`load_project_agents` / `load_user_agents`):
   - `.agents/agents` wins over `.HRAgent/agents` for the same agent name.
...
- Docs PR tracking this feature: https://github.com/HRAgents/docs/pull/358
