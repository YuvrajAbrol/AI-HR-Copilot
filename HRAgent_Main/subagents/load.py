...
`.agents/agents` or `.HRAgent/agents` directories at the project or user level.
...
Directory convention (in priority order):
    ...
    ~/.HRAgent/               # User-level, legacy (lowest file priority)
        agents/
            my-global-agent.md
...
Priority (highest to lowest):
  ...
  4. Project-level `.HRAgent/agents/*.md`
  5. User-level `~/.agents/agents/*.md`
  6. User-level `~/.HRAgent/agents/*.md`
"""
...
def load_project_agents(project_dir: str | Path) -> list[AgentDefinition]:
    ...
    Searches for
        ...
        - project_dir/.HRAgent/agents (in that order).
...
def discover_agents(
    ...
    *,
    ...
    ``HRAgent-tools`` (not an SDK dependency), so callers wanting them add
