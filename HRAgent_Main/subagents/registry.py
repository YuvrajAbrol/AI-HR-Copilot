...
Example usage:
    from subagents import register_agent
...
from subagents.load import (
    load_project_agents,
    load_user_agents,
)
from subagents.schema import AgentDefinition
from utilities.deprecation import warn_deprecated
...
def agent_definition_to_factory(
    ...
    if agent_def.skills:
        from skills import load_available_skills
...
def register_file_agents(work_dir: str | Path) -> list[str]:
    ...
    `.HRAgent/agents`, and user-level `~/.agents/agents` and `~/.HRAgent/agents`
    directories.

    Project-level definitions take priority over user-level ones, and within
    each level `.agents/` takes priority over `.HRAgent/`.
