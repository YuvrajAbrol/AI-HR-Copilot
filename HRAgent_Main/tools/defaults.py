"""Canonical default tool names for the standard HRAgents agent.
...
the canonical defaults here lets ``HRAgent-sdk`` (which must not import
``HRAgent-tools``) default a toolset from data alone — ``Tool`` is a spec
(name + params) resolved to an implementation only at runtime via the registry.

``tools.preset.default.get_default_tools`` remains the constructor
...
"""Names of the standard exec tools every default HRAgents agent gets."""
...
def default_tool_specs(
    *,
    ...
    """Default tool specs for an HRAgents agent whose settings carry no tools.
