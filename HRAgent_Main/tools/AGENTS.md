...
- This directory (`HRAgent-tools/HRAgent/tools/`) contains runtime tool implementations under the `HRAgent.tools.*` namespace.
...
- Treat `HRAgent-tools/HRAgent/tools/__init__.py` as the published surface for `HRAgent-tools`; `__all__` is considered public API.
