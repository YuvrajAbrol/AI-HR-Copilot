...
# Canonical planning tool names (implementations were removed during the
...
def register_planning_tools() -> None:
    ...
    logger.debug("No planning tool implementations to register.")
...
def get_planning_tools(plan_path: str | None = None) -> list[Tool]:
    ...
    Args:
        plan_path: Optional absolute path to PLAN.md file. If provided, will be
            passed to the planning file editor tool via params.
    ...
    """
    tools = [Tool(name=name) for name in PLANNING_TOOL_NAMES]

    # Build params for planning_file_editor if plan_path is provided
    if plan_path:
        tools[-1] = Tool(name="planning_file_editor", params={"plan_path": plan_path})
