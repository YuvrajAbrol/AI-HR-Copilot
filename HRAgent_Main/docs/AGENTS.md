...
- HRAgents provider LLMs keep public config as `HRAgent/<model>` and translate to `litellm_proxy/<model>` plus the HRAgents proxy `api_base` only at LiteLLM boundaries. Legacy persisted proxy payloads are migrated once at settings/profile load boundaries; do not add downstream UI reverse-mapping for the normal path.
- Programmatic settings live in `HRAgent-sdk/HRAgent/sdk/settings/`. Treat `AgentSettings` and `export_settings_schema()` as the canonical structured settings surface in the SDK, and keep that schema focused on neutral config structure and semantics; downstream apps should own client-specific ordering, icons, widgets, and slash-command presentation.
...
- `mcp_config` is declared on `HRAgentsAgentSettings` and `ACPAgentSettings` (and inherited by `LLMAgentSettings`) — the variants of the `AgentSettingsConfig` union. It is a flat `dict[str, MCPServer]` server map, where `MCPServer` is the SDK's own model in `HRAgent-sdk/HRAgent/sdk/mcp/config.py` — not FastMCP's `MCPConfig`, and not the `.mcp.json`-style `{"mcpServers": {...}}` wrapper (the v4→v5 migration `_migrate_mcp_config_to_server_map` unwraps that key). FastMCP's typed `MCPConfig` is built only at the FastMCP boundary, in `_prepare_mcp_config()` in `HRAgent-sdk/HRAgent/sdk/mcp/utils.py`, from the payload produced by `to_fastmcp_mcp_config()` in `HRAgent-sdk/HRAgent/sdk/mcp/config.py`. Keep serialized MCP output compact with `exclude_none=True, exclude_defaults=True`, applied per-`MCPServer` in `HRAgent-sdk/HRAgent/sdk/mcp/config.py`.
- Persisted SDK settings should use the direct `model_dump()` shape with a top-level `schema_version`; avoid adding wrapped payload formats or legacy migration shims in `HRAgent/sdk/settings/model.py`.
...
- LLM-specific behavior tweaks should start in `HRAgent-sdk/HRAgent/sdk/llm/utils/model_features.py` whenever they can be expressed as model/provider capabilities. Genuinely try to keep provider-specific criteria out of `HRAgent-sdk/HRAgent/sdk/llm/llm.py`; only touch `llm.py` when the behavior cannot be represented cleanly in the feature registry or a focused helper.
- AgentSkills progressive disclosure resolves in `AgentContext._resolve_dynamic_data()`, shared by `get_system_message_suffix()` and the prompt section registry (`HRAgent/sdk/context/prompts/sections/`), and lands in `<available_skills>`. `HRAgent.sdk.skills.to_prompt()` (`HRAgent-sdk/HRAgent/sdk/skills/skill.py`) truncates each prompt description to 1024 characters because the AgentSkills specification caps `description` at 1-1024 characters.
...
- The duplicate-issue automation scripts should validate `owner/repo` arguments before interpolating GitHub API paths, handle per-issue auto-close failures without aborting the whole batch, and keep `app_conversation_id` paths unquoted because HRAgents conversation IDs are already canonicalized for those endpoints.
- GitHub Actions workflows pin external third-party actions to a full 40-character commit SHA, with the version tag in a trailing comment (`uses: owner/repo@<sha> # v1.2.3`); mutable tags or branches are not used for third-party actions. GitHub-authored (`actions/*`, `github/*`) and first-party (`HRAgents/*`) refs are currently exempt. Dependabot's `github-actions` ecosystem still bumps the pinned SHA (and the trailing comment), so pinning does not block security or version updates.

- `agent-server` now defaults `TMUX_TMPDIR` to a per-process directory under the system temp dir (`HRAgent-agent-server-<pid>`) when the environment variable is unset. This isolates tmux sockets/cleanup across concurrent server instances while still respecting an explicit `TMUX_TMPDIR` override.
...
- Agent-server Docker publish tags are defined centrally in `HRAgent-agent-server/HRAgent/agent_server/docker/build.py`; keep `server.yml` manifest publication derived from the emitted per-arch tags so SHA/branch/git-tag aliases stay in sync, while preserving the legacy `latest-<variant>` alias used by workspace defaults.
- The published agent-server Docker images in `.github/workflows/server.yml` must pass `HRAgentS_BUILD_GIT_SHA` and `HRAgentS_BUILD_GIT_REF` as explicit `docker/build-push-action` build args; the workflow only uses `docker/build.py` for context/tag generation, so those runtime env vars are otherwise left at the Dockerfile `unknown` defaults.
- The PyInstaller agent-server binary should copy HRAgents distribution metadata (`HRAgent-agent-server`, `HRAgent-sdk`, `HRAgent-tools`, `HRAgent-workspace`) in `HRAgent-agent-server/HRAgent/agent_server/agent-server.spec`, otherwise `/server_info` version lookups via `importlib.metadata` can fall back to `unknown` inside published binary images.
- Agent-server deferred init (warm-pool / dormant mode) is driven by `Config.deferred_init` (env `OH_DEFERRED_INIT`). The `InitService` in `HRAgent-agent-server/HRAgent/agent_server/init_router.py` owns the dormant→initializing→ready transition and is registered on `app.state.init_service` only when `deferred_init=True`; the `require_initialized` dependency, added to the `/api/*` router, returns 503 while not `ready`. Bootstrap auth for `POST /api/init` uses the existing `secret_key` (`X-Init-API-Key` header) — the orchestrator already holds this key for encryption, and it is overwritten when the per-user runtime config arrives in the init body. The agent-server's 5xx exception handler rewrites `detail` on 503s, so warm-pool orchestrators should rely on the HTTP status code (not the body) when probing dormant state.
...
- Tool availability filtering belongs in `HRAgent-sdk/HRAgent/sdk/tool/registry.py` via `list_usable_tools()`, which preserves registration order and defaults tools to usable unless they expose an `is_usable()` callable. Environment-specific checks like Chromium detection should live on the concrete tool class (`BrowserToolSet.is_usable()`), while agent-server surfaces such as `/server_info` should consume the registry helper rather than re-implement per-tool filtering.
- Pydantic secret field helpers live in `HRAgent-sdk/HRAgent/sdk/utils/pydantic_secrets.py`. `serialize_secret()` handles serialization (cipher / `expose_secrets` / default Pydantic masking); `validate_secret()` handles deserialization (cipher decryption, redacted/empty → `None`); `is_redacted_secret()` checks for the sentinel; `REDACTED_SECRET_VALUE` is the canonical sentinel string. For `dict[str, str]` fields whose values are all secrets, wrap each value in `SecretStr` and call `serialize_secret` per value (see `LookupSecret._serialize_secrets`). Do not hand-roll redaction logic in field serializers.

- `LookupSecret` normalizes hostless URLs against `OH_INTERNAL_SERVER_URL` (set by `HRAgent-agent-server.__main__` from the bound host/port, rewriting wildcard binds to loopback) and otherwise falls back to `http://127.0.0.1:8000`, so relative secret URLs can safely target the current agent-server instance.
...
- SDK: [HRAgent-sdk/HRAgent/sdk/AGENTS.md](HRAgent-sdk/HRAgent/sdk/AGENTS.md)
- Subagents: [HRAgent-sdk/HRAgent/sdk/subagent/AGENTS.md](HRAgent-sdk/HRAgent/sdk/subagent/AGENTS.md)
- Tools: [HRAgent-tools/HRAgent/tools/AGENTS.md](HRAgent-tools/HRAgent/tools/AGENTS.md)
- Workspace: [HRAgent-workspace/HRAgent/workspace/AGENTS.md](HRAgent-workspace/HRAgent/workspace/AGENTS.md)
- Agent server: [HRAgent-agent-server/AGENTS.md](HRAgent-agent-server/AGENTS.md)

## API compatibility pointers

- For SDK Python API deprecation/removal policy, read
  [HRAgent-sdk/HRAgent/sdk/AGENTS.md](HRAgent-sdk/HRAgent/sdk/AGENTS.md).
...
- For public REST APIs, read
  [HRAgent-agent-server/AGENTS.md](HRAgent-agent-server/AGENTS.md).
...
- In every commit message, you should add "Co-authored-by: HRAgent <HRAgent@all-hands.dev>"
...
- Put unit tests under the corresponding domain folder in `tests/` (e.g., `tests/sdk`, `tests/tools`, `tests/workspace`). For example, changes to `HRAgent-sdk/HRAgent/sdk/tool/tool.py` should be covered in `tests/sdk/tool/test_tool.py`.
...
The browser session recording tool saves recordings to `.agent_tmp/browser_observations/recording-{timestamp}/` (see `BROWSER_RECORDING_OUTPUT_DIR` in `HRAgent-tools/HRAgent/tools/browser_use/definition.py`).
...
<REPO>
<PROJECT_STRUCTURE>
- This is a `uv`-managed Python monorepo (single `uv.lock` at repo root) with multiple distributable packages: `HRAgent-sdk/` (SDK), `HRAgent-tools/` (built-in tools), `HRAgent-workspace/` (workspace impls), and `HRAgent-agent-server/` (server runtime).
...
- Python namespace is `HRAgent.*` across packages; keep new modules within the matching package and mirror test paths under `tests/`.
</PROJECT_STRUCTURE>
...
- Run agent-server stress tests: `uv run pytest -m stress` (see [HRAgent-agent-server/AGENTS.md](HRAgent-agent-server/AGENTS.md))
- Build agent-server: `make build-server` (output: `dist/agent-server/`)
- Clean caches: `make clean`
- Run SDK examples: see [HRAgent-sdk/HRAgent/sdk/AGENTS.md](HRAgent-sdk/HRAgent/sdk/AGENTS.md).
