...
Following the same pattern as HRAgents app-server's FileSettingsStore
...
from configuration.profiles.agent_profile_store import AgentProfileStore
...
DEFAULT_PERSISTENCE_DIR = Path("workspace/.HRAgent")
...
def _get_persistence_dir(config: Config | None = None) -> Path:
    ...
    if config is not None:
        return config.conversations_path.parent / ".HRAgent"
...
def _get_profile_persistence_dir() -> Path:
    ...
    ``OH_PERSISTENCE_DIR`` they fall back to the user's ``~/.HRAgent``
...
def get_llm_profile_store() -> LLMProfileStore:
    ...
    when set, else ``~/.HRAgent``. This honors isolated agent-server
