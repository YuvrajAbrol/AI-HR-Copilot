...
from runtime.telemetry.sanitizer import (
    UNKNOWN_TOKEN,
    pseudonymize,
    safe_token,
    safe_version,
...
DISTINCT_ID_HEADER: Final[str] = "X-HRAgents-Telemetry-Distinct-Id"

_MAX_DISTINCT_ID_LEN: Final[int] = 256


def distinct_id_from_header(value: str | None) -> str | None:
    """Coerce the ``X-HRAgents-Telemetry-Distinct-Id`` header, or ``None``.
...
def build_runtime_properties(*, deferred_init: bool) -> RuntimeProperties:
    ...
    # Imported lazily: server_details_router pulls in the tool registry, which
    # would otherwise create an import cycle through models.llm at load time.
    from runtime.server.server_details_router import ServerInfo
