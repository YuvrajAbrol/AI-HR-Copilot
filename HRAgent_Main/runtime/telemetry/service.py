...
from runtime.telemetry.policy import TelemetryDecision, resolve
from runtime.telemetry.sink import (
...
def _read_misc_settings_sync(config: Config) -> dict[str, Any]:
    from runtime.persistence.store import get_settings_store
...
def _build_exporter(config: Config) -> TelemetryExporter | None:
    ...
    if spec.exporter == "posthog":
        ...
        try:
            ...
            from runtime.telemetry.posthog_exporter import (
                PostHogExporter,
            )
            ...
            logger.warning(
                ...
                "telemetry is inactive. Install HRAgent-agent-server[posthog]."
            )
            return None
    ...
    if spec.exporter == "http":
        if not spec.http_endpoint:
            ...
            return None
        from runtime.telemetry.http_exporter import HttpExporter
