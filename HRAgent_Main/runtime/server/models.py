...
from core.conversation.request import (  # re-export for backward compat
...
from core.conversation.types import ConversationTags
from core.execution.event.base import Event
from core.execution.hooks import HookConfig
from models.llm.message import (  # re-export
    ImageContent as ImageContent,
    TextContent as TextContent,
)
from models.llm.utils.metrics import MetricsSnapshot
...
from utilities import HRAgentsUUID, utc_now
...
class StoredConversation(StartConversationRequest):
    ...
    forked_from_conversation_id: HRAgentsUUID | None = Field(
        default=None,
        description=(
...
class _ConversationInfoBase(BaseModel):
    ...
    created_at: datetime = Field(default_factory=utc_now)
    updated_at: datetime = Field(default_factory=utc_now)
    # Plain ``UUID`` (not ``HRAgentsUUID``) so it JSON-serializes dashed, exactly
    ...
    current_model_id: str | None = Field(
        ...
        description=(
            ...
            "still initializing. Native HRAgents agents leave this ``None`` — "
    ...
    available_models: list[ACPModelInfo] = Field(
        ...
        description=(
            ...
            "the (UNSTABLE) capability and for native HRAgents agents. "
    ...
    supports_runtime_model_switch: bool = Field(
        ...
        description=(
            ...
            "``False`` for native "
            "HRAgents agents, for a known provider that declares no support, "
...
def trim_conversation_response_skills(info: ConversationInfo) -> ConversationInfo:
    ...
    actually reads from the HTTP response (agent-canvas, HRAgents
...
class BashEventBase(DiscriminatedUnionMixin, ABC):
    ...
    id: HRAgentsUUID = Field(default_factory=uuid4)
...
class BashEventPage(HRAgentsModel):
