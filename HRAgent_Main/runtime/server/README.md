# HRAgents Agent Server

The HRAgents Agent Server is a minimal REST API and WebSocket server that provides a programmatic interface for interacting with HRAgents AI agents. It uses the local filesystem to store conversations, events, and workspace files, making it ideal for development, testing, and lightweight deployments.
...
uv run python -m HRAgent.agent_server

# Start with custom host and port
uv run python -m HRAgent.agent_server --host localhost --port 3000

# Start with auto-reload (for dev)
uv run python -m HRAgent.agent_server --reload
```
...
|----------|-------------|---------|
| `HRAgentS_AGENT_SERVER_CONFIG_PATH` | Path to JSON configuration file | `workspace/HRAgent_agent_server_config.json` |
...
Create a JSON configuration file (default: `workspace/HRAgent_agent_server_config.json`):
...
sending the frontend's PostHog id in the `X-HRAgents-Telemetry-Distinct-Id`
header (`headers["X-HRAgents-Telemetry-Distinct-Id"] = posthog.get_distinct_id()`).
...
```bash
pip install 'HRAgent-agent-server[posthog]'
```
...
```
workspace/
├── HRAgent_agent_server_config.json    # Configuration file
...
# Run with coverage
uv run pytest tests/agent_server/ --cov=HRAgent.agent_server
```
...
3. **Configuration not found**: Check the `HRAgentS_AGENT_SERVER_CONFIG_PATH` environment variable
