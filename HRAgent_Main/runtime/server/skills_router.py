"""Skills router for HRAgents Agent Server.
...
from runtime.server.skills_service import (
    ExposedUrlData,
...
from plugins.marketplace import MarketplaceRegistration
from skills import (
    InstalledSkillInfo,
    SkillFetchError,
    SkillValidationError,
)
from skills.skill import DEFAULT_MARKETPLACE_PATH
from skills.utils import SKILL_NAME_PATTERN
...
class SkillsRequest(BaseModel):
    ...
    load_public: bool = Field(
        default=True, description="Load public skills from HRAgents/extensions repo"
    )
    load_user: bool = Field(
        default=True, description="Load user skills from ~/.HRAgent/skills/"
    )
...
class InstallSkillRequest(BaseModel):
    ...
    source: str = Field(
        ...
        description=(
            ...
            "Examples: "
            "'https://github.com/HRAgents/extensions/tree/main/skills/github', "
            "'github:HRAgents/extensions/skills/github', "
            "'/path/to/skill'"
        ),
    )
    ref: str | None = Field(
...
def get_skills(request: SkillsRequest, http_request: Request) -> SkillsResponse:
    ...
    2. Public skills - From GitHub HRAgents/extensions repository
    3. User skills - From ~/.HRAgent/skills/
    4. Organization skills - From {org}/.HRAgent or equivalent
    5. Project skills (highest) - From {workspace}/.HRAgent/skills/
...
def sync_skills() -> SyncResponse:
    ...
    the latest skills from the HRAgents/extensions repository.
...
def install_skill_endpoint(request: InstallSkillRequest) -> InstalledSkillResponse:
    ...
    the user's installed skills directory (~/.HRAgent/skills/installed/).
...
def list_installed_skills_endpoint() -> InstalledSkillsListResponse:
    ...
    directory (~/.HRAgent/skills/installed/).
...
def get_marketplace_catalog() -> MarketplaceCatalogResponse:
    ...
    Returns a list of available skills from the HRAgents extensions
