"""Plugins service for HRAgents Agent Server.
...
* Installed-plugin management — thin wrappers over the SDK's installed-plugins
  subsystem (``plugins``) — plus listing the locally-available
  plugins.
* The *plugins-only* marketplace catalog. It returns only true plugins from the
  HRAgents extensions marketplace — entries whose ``source`` lives under
...
def service_list_available_plugins(
    ...
    auto-load" ticket (``plugins.discovery``). It is imported lazily
    so this module imports cleanly before that ticket is merged; this endpoint
    becomes functional once it lands.
    """
    from plugins import load_available_plugins  # type: ignore
...
# The HRAgents extensions marketplace lists both skills and true plugins under
...
def _fetch_plugin_catalog_entries(
    ...
    # ``.plugin/`` or ``.claude-plugin/`` — the real HRAgents/extensions layout,
