...
from tools.registry import list_usable_tools
...
class ServerInfo(BaseModel):
    uptime: float
    idle_time: float
    title: str = "HRAgents Agent Server"

    version: str = Field(default_factory=lambda: _package_version("HRAgent"))
    sdk_version: str = Field(default_factory=lambda: _package_version("HRAgent"))
    tools_version: str = Field(default_factory=lambda: _package_version("HRAgent"))
    workspace_version: str = Field(
        default_factory=lambda: _package_version("HRAgent")
    )

    build_git_sha: str = Field(
        default_factory=lambda: os.environ.get("HRAgentS_BUILD_GIT_SHA", "unknown")
    )
    build_git_ref: str = Field(
        default_factory=lambda: os.environ.get("HRAgentS_BUILD_GIT_REF", "unknown")
    )
    python_version: str = Field(default_factory=lambda: sys.version)
    usable_tools: list[str] = Field(default_factory=lambda: list_usable_tools())
    # Terminal timeout reporting was removed with the terminal runtime.
    runtime_idle_timeout_seconds: float | None = Field(default=None)
    ...
    max_foreground_terminal_timeout_seconds: float | None = Field(default=None)
