...
reference-bearing :class:`~configuration.profiles.AgentProfile` union and keeps a
...
MCP references and returns :class:`~configuration.profiles.AgentProfileDiagnostics`
...
from runtime.server._secrets_exposure import get_cipher, get_config
...
from runtime.server.profiles_router import MAX_PROFILES, _has_api_key
...
from configuration.profiles.agent_profile_store import PROFILE_NAME_PATTERN
...
async def materialize_agent_profile(
    ...
    # (catalog minus ``disabled_skills``) resolve. Only HRAgents profiles carry
