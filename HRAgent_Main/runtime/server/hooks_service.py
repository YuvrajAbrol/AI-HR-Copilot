"""Hooks service for HRAgents Agent Server.
...
Hook Sources:
- Project hooks: {workspace}/.HRAgent/hooks.json
- User hooks: ~/.HRAgent/hooks.json (future)
"""
...
def load_hooks_from_workspace(project_dir: str | None = None) -> HookConfig | None:
    """Load hooks from the workspace .HRAgent/hooks.json file.

    This function reads the hooks configuration from the project's
    .HRAgent/hooks.json file if it exists.
    ...
    hooks_path = Path(project_dir) / ".HRAgent" / "hooks.json"
