...
from runtime.server.config import Config
from runtime.server.conversation_service import ConversationService
from runtime.server.dependencies import get_conversation_service
from runtime.integrations.openai.models import (
    OpenAIChatCompletionRequest,
    OpenAIChatCompletionResponse,
    OpenAIModelListResponse,
)
from runtime.integrations.openai.service import (
...
def check_openai_api_key(
    ...
    """Accept the same session key through HRAgents and OpenAI auth shapes.
...
def _parse_observability_overrides(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="X-HRAgents-Observability-Metadata must be a JSON object",
        ) from exc
...
async def create_chat_completion(
    ...
    request: Request,
    response: Response,
    x_HRAgent_server_conversation_id: Annotated[
        UUID | None, Header(alias="X-HRAgents-ServerConversation-ID")
    ] = None,
    x_HRAgent_observability_span_name: Annotated[
        str | None, Header(alias="X-HRAgents-Observability-Span-Name")
    ] = None,
    x_HRAgent_observability_tags: Annotated[
        str | None, Header(alias="X-HRAgents-Observability-Tags")
    ] = None,
    x_HRAgent_observability_metadata: Annotated[
        str | None, Header(alias="X-HRAgents-Observability-Metadata")
    ] = None,
    ...
    result = await run_chat_completion(
        ...
        reusable_conversation_id=x_HRAgent_server_conversation_id,
        observability_overrides=_parse_observability_overrides(
            span_name=x_HRAgent_observability_span_name,
            tags=x_HRAgent_observability_tags,
            metadata=x_HRAgent_observability_metadata,
        ),
    )
    ...
    if body.stream:
        ...
        return StreamingResponse(
            ...
            headers={"X-HRAgents-ServerConversation-ID": conversation_id},
        )

    response.headers["X-HRAgents-ServerConversation-ID"] = conversation_id
