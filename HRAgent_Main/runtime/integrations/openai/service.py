...
from runtime.server.event_service import EventService
...
from core.conversation.request import (
    SendMessageRequest,
...
from models.llm.message import ImageContent, TextContent
from configuration.settings import ACPAgentSettings, HRAgentsAgentSettings
from core.workspace import LocalWorkspace


_MODEL_PREFIX = "HRAgent_"
...
def _profile_name_from_model(model: str) -> str:
    ...
    raise HTTPException(
        ...
        detail=f"Unknown HRAgents model '{model}'. Use GET /v1/models.",
    )
...
def _with_profile_llm_and_system_text(
    agent_settings: HRAgentsAgentSettings | ACPAgentSettings,
    llm: LLM,
    system_text: str,
    ...
    if isinstance(updated, HRAgentsAgentSettings):
