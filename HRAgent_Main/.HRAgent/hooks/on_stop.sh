...
#   HRAgentS_PROJECT_DIR - Project directory
#   HRAgentS_SESSION_ID - Session ID
#   GITHUB_TOKEN - GitHub API token (if available)

set -o pipefail

PROJECT_DIR="${HRAgentS_PROJECT_DIR:-$(pwd)}"
...
if [ -n "$CHANGED_FILES" ]; then
    ...
    while IFS= read -r file; do
        case "$file" in
            HRAgent-sdk/*) add_project "tests/sdk" ;;
            HRAgent-tools/*) add_project "tests/tools" ;;
            HRAgent-workspace/*) add_project "tests/workspace" ;;
            HRAgent-agent-server/*) add_project "tests/agent_server" ;;
