...
- ``HRAgent-tools`` for command splitting and escape doubling
  (``terminal.utils.command``).
- ``HRAgent-sdk`` for AST-aware security detection
  (the planned ``security.policies.defense_in_depth.*`` analyzers).

Hosting the parser inside ``HRAgent-sdk`` lets both consumers depend
...
``HRAgent-tools/HRAgent/tools/terminal/utils/command.py``: sharing
