...
Example (bare guardrail):
    ...
    >>> from security import ToolShieldLLMSecurityAnalyzer
...
Example (with ToolShield tool-specific safety experiences):
    Install the optional extra ``pip install HRAgent-sdk[toolshield]``
...
from core.execution.event import ActionEvent
...
# per the actor's system prompt (see core.execution.event.llm_convertible.
...
class ToolShieldLLMSecurityAnalyzer(SecurityAnalyzerBase):
    ...
    ``pip install HRAgent-sdk[toolshield]`` and pass the rendered
    ...
    safety_experiences: str = Field(
        ...
        description=(
            ...
            "(``pip install HRAgent-sdk[toolshield]``). Callers with "
