...
from core.conversation.request import SendMessageRequest
...
if TYPE_CHECKING:
    from core.agent import ACPAgent, Agent
...
class SettingsFieldSchema(BaseModel):
    ...
    variant: str | None = Field(
        ...
        description=(
            ...
            "variant (``'HRAgent'`` or ``'acp'``). The GUI filters fields by the "
...
class SettingsSectionSchema(BaseModel):
    ...
    variant: str | None = Field(
        ...
        description=(
            ...
            "variant (e.g. ``'HRAgent'`` or ``'acp'``). The GUI filters sections by "
...
class NoOpCondenserSettings(CondenserSettings):
    ...
    def build_condenser(self, llm: LLM) -> CondenserBase | None:  # noqa: ARG002
        ...
        from context.condenser import NoOpCondenser
...
class AgentSettingsBase(BaseModel):
    ...
    either the :class:`HRAgentsAgentSettings` or :class:`ACPAgentSettings`
    ...
    def create_agent(self) -> AgentBase:
        ...
        Subclasses (:class:`HRAgentsAgentSettings`, :class:`ACPAgentSettings`)
        override this to return the appropriate
        :class:`~core.agent.base.AgentBase` subclass.
...
def _migrate_agent_settings_v1_to_v2(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Before the v1.19.0 ``LLMAgentSettings`` → ``HRAgentsAgentSettings`` rename,
    persisted payloads carried ``agent_kind: 'llm'``. The two classes are
    field-compatible (``LLMAgentSettings`` is a subclass of
    ``HRAgentsAgentSettings`` that only narrows the discriminator literal),
    ...
    ``HRAgentsAgentSettings`` (the canonical class) accept legacy data
    without losing any fields.
    """
    migrated = dict(payload)
    migrated["schema_version"] = 2
    if migrated.get("agent_kind") == "llm":
        migrated["agent_kind"] = "HRAgent"
    return migrated
...
def _migrate_agent_settings_v3_to_v4(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(llm, dict):
        migrated["llm"] = canonicalize_HRAgent_llm_payload(llm)
...
class ConversationSettings(BaseModel):
    ...
    agent_settings: AgentSettingsConfig | None = Field(
        ...
        description=(
            ...
            "``HRAgentsAgentSettings`` or ``ACPAgentSettings`` variant."
        ),
    )
...
AgentKind = Literal["HRAgent", "llm", "acp"]
...
:data:`~configuration.settings.acp_providers.ACP_PROVIDERS`.
"""


class HRAgentsAgentSettings(AgentSettingsBase):
    ...
    agent_kind: Literal["HRAgent"] = Field(
        default="HRAgent",
        description=(
            "Discriminator for the ``AgentSettings`` union. ``'HRAgent'`` selects "
            "the standard built-in HRAgents agent."
        ),
    )
    agent: str = Field(
    ...
    tools: list[Tool] | None = Field(
        ...
        description=(
            ...
            "standard exec set (see tools.defaults), plus the "
    ...
    def create_agent(self) -> Agent:
        ...
        Example::
            ...
            settings = HRAgentsAgentSettings(
        ...
        from tools.builtins import BUILT_IN_TOOLS, SwitchLLMTool
    ...
    def build_critic(self) -> CriticBase | None:
        """Create a critic from these settings.

        The ``APIBasedCritic`` implementation was removed during the cleanup,
        so no critic is built; this always returns ``None``. The
        ``verification.*`` critic settings are preserved so the model remains
        loadable, but nothing consumes them anymore.
        """
        return None
...
class ACPAgentSettings(AgentSettingsBase):
    ...
    :class:`HRAgentsAgentSettings` do not apply here.
    ...
    mcp_config: dict[str, MCPServer] = Field(
        ...
        description=(
            ...
            "HRAgents agent — where these become in-process MCP tools — the "
    ...
    acp_isolate_data_dir: bool = Field(
        ...
        description=(
            ...
            ":attr:`~core.agent.ACPAgent.acp_isolate_data_dir`; off by "
    ...
    def api_key_env_var(self) -> str | None:
        ...
        Delegates to the :data:`~configuration.settings.acp_providers.ACP_PROVIDERS`
    ...
    def resolve_acp_command(self) -> list[str]:
        ...
        up the default from :data:`~configuration.settings.acp_providers.ACP_PROVIDERS`.
...
class LLMAgentSettings(HRAgentsAgentSettings):
    """Legacy ``agent_kind='llm'`` variant of :class:`HRAgentsAgentSettings`.

    ``LLMAgentSettings`` was the public class name before the v1.19.0 rename.
    The public import aliases (``from HRAgent.sdk import LLMAgentSettings`` and
    ``from configuration.settings import LLMAgentSettings``) were removed in
    v1.24.0 — use :class:`HRAgentsAgentSettings` for all new code.

    The class itself is retained (reachable at
    ``configuration.settings.model.LLMAgentSettings``) because it remains a
...
def _agent_settings_discriminator(value: Any) -> str:
    ...
    """
    if isinstance(value, BaseModel):
        return getattr(value, "agent_kind", "HRAgent")
    if isinstance(value, dict):
        return value.get("agent_kind", "HRAgent")
    return "HRAgent"


AgentSettingsConfig = Annotated[
    Annotated[HRAgentsAgentSettings, Tag("HRAgent")]
...
_AGENT_SETTINGS_ADAPTER: TypeAdapter[
    HRAgentsAgentSettings | LLMAgentSettings | ACPAgentSettings
...
def validate_agent_settings(
    ...
    *,
    ...
    ``HRAgentsAgentSettings`` rename.
    """
    if isinstance(data, HRAgentsAgentSettings | ACPAgentSettings):
        return data
    payload = _apply_persisted_migrations(
        data,
    ...
    # always returns a ``{HRAgent, acp}`` variant.
    if payload.get("agent_kind") == "llm":
        payload["agent_kind"] = "HRAgent"
...
def apply_agent_settings_diff(
    ...
    *,
    context: Mapping[str, Any] | None = None,
) -> HRAgentsAgentSettings | ACPAgentSettings:
    ...
    * When ``diff`` changes ``agent_kind``, start from a *fresh* base for the
      ...
      validation (ACP's nullable ``agent_context`` is invalid for HRAgents) or
...
def default_agent_settings() -> HRAgentsAgentSettings:
    """Return a default :class:`HRAgentsAgentSettings` instance.
    ...
    """
    return HRAgentsAgentSettings()


def create_agent_from_settings(
    settings: HRAgentsAgentSettings | ACPAgentSettings,
) -> AgentBase:
    """Dispatch to the variant's ``create_agent()`` method.

    Returns either :class:`~core.agent.Agent` (LLM variant) or
    :class:`~core.agent.ACPAgent` (ACP variant).
    """
...
def export_agent_settings_schema() -> SettingsSchema:
    ...
    carry a ``variant`` tag (``'HRAgent'``, ``'acp'``, or ``None`` for
    shared) so the frontend can filter by the page's variant.
    """
    llm_schema = HRAgentsAgentSettings.export_schema()
    ...
    _merge(llm_schema, default_variant="HRAgent")
