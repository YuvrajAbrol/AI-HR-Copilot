...
Callers outside the SDK (e.g. ``HRAgent-agent-server``, the ``HRAgents``
frontend, and the ``@HRAgent/typescript-client`` mirror) can import
...
class ACPFileSecretSpec(BaseModel):
    ...
    :attr:`secret_name` — and :class:`~core.agent.ACPAgent` materialises
    ...
    :attr:`~core.agent.ACPAgent.acp_file_secrets` to support other ACP
...
class ACPProviderInfo:
    ...
    pointing at it) that :class:`~core.agent.ACPAgent` materialises
    ...
    :meth:`~configuration.settings.model.ACPAgentSettings.resolve_acp_command`
    ...
    :attr:`~core.agent.ACPAgent.acp_isolate_data_dir`.
    """
...
# HRAgent-agent-server/HRAgent/agent_server/docker/Dockerfile — a bump must
...
def default_acp_file_secrets() -> tuple[ACPFileSecretSpec, ...]:
    ...
    :attr:`~core.agent.ACPAgent.acp_file_secrets`, which a downstream
