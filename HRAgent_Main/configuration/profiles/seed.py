"""Build the default :class:`~configuration.profiles.AgentProfile` from live settings.

The inverse of :func:`~configuration.profiles.resolve_agent_profile`: turns the
...
if TYPE_CHECKING:
    from configuration.settings import AgentSettingsConfig
...
def build_seed_profile(
    ...
    *,
    ...
    ``mcp_server_refs=None`` exposes all of the user's MCP servers; an HRAgents
