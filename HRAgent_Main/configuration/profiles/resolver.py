...
:data:`~configuration.settings.model.AgentSettingsConfig` embeds the resolved
...
from configuration.profiles.agent_profile import (
    ACPAgentProfile,
    HRAgentsAgentProfile,
)
from configuration.settings.acp_providers import get_acp_provider
...
from utilities.pydantic_secrets import REDACTED_SECRET_VALUE


if TYPE_CHECKING:
    from models.llm.llm import LLM
    from models.llm.llm_profile_store import LLMProfileLoader
...
class AgentProfileDiagnostics(BaseModel):
    ...
    # Skill selection (HRAgents only). ``disabled_skills`` is a deny-list over
...
def _build_HRAgent_settings(
    profile: HRAgentsAgentProfile,
    llm: LLM,
    mcp_config: dict[str, MCPServer],
    filtered_skills: list[Skill],
) -> AgentSettingsConfig:
    """Compose the resolved ``HRAgentsAgentSettings`` from a profile + LLM.
...
def resolve_agent_profile(
    profile: HRAgentsAgentProfile | ACPAgentProfile,
    *,
    ...
    HRAgents profile keeps all of it except the names in ``disabled_skills``.
    ...
    Raises:
        ProfileNotFound: ``llm_profile_ref`` does not exist (HRAgents path).
    ...
    if isinstance(profile, HRAgentsAgentProfile):
        ...
        return _build_HRAgent_settings(profile, llm, filtered_mcp, filtered_skills)
...
def resolve_agent_profile_dry_run(
    ...
    *,
    ...
    # Skill selection report (HRAgents only; ACP injects no user/public skills).
    ...
    if diagnostics.valid:
        ...
        try:
            if isinstance(profile, HRAgentsAgentProfile):
                ...
                if llm is None:
                    raise RuntimeError(
                        "HRAgents profile marked valid without a resolved LLM"
                    )
                settings = _build_HRAgent_settings(
