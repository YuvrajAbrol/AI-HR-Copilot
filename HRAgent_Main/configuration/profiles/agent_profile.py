...
:data:`~configuration.settings.model.AgentSettingsConfig`: the profile carries
...
from configuration.settings.model import (
    ACPServerKind,
...
class ProfileVerificationSettings(BaseModel):
    ...
    :class:`~configuration.settings.model.VerificationSettings`: ``critic_api_key``
...
class AgentProfileBase(BaseModel):
    ...
    (or an HRAgents payload carrying ``acp_*``) is rejected rather than
    ...
    # selects them by *exclusion* (``HRAgentsAgentProfile.disabled_skills``, a
...
class HRAgentsAgentProfile(AgentProfileBase):
    """``agent_kind="HRAgent"`` profile — references an LLM profile by name.

    Mirrors the configurable surface of
    :class:`~configuration.settings.model.HRAgentsAgentSettings`, except the
    ...
    agent_kind: Literal["HRAgent"] = Field(
        default="HRAgent",
        description=(
            "Discriminator for the ``AgentProfile`` union. ``'HRAgent'`` "
            "selects the standard built-in HRAgents agent."
        ),
    )
...
class ACPAgentProfile(AgentProfileBase):
    ...
    :data:`~configuration.settings.acp_providers.ACP_PROVIDERS` registry. The
...
def _agent_profile_discriminator(value: Any) -> str:
    """Discriminator for :data:`AgentProfile` — defaults to ``'HRAgent'``.

    A payload without an explicit ``agent_kind`` is treated as the standard
    HRAgents variant, mirroring
    :func:`~configuration.settings.model._agent_settings_discriminator`.
    """
...
AgentProfile = Annotated[
    Annotated[HRAgentsAgentProfile, Tag("HRAgent")]
...
def _migrate_v1_to_v2(payload: dict[str, Any]) -> dict[str, Any]:
    if (
        payload.get("agent_kind", "HRAgent") == "HRAgent"
...
_AGENT_PROFILE_ADAPTER: TypeAdapter[HRAgentsAgentProfile | ACPAgentProfile] = (
...
def validate_agent_profile(
    ...
    *,
...
) -> HRAgentsAgentProfile | ACPAgentProfile:
    ...
    """
    if isinstance(data, HRAgentsAgentProfile | ACPAgentProfile):
        return data
