...
An ``HRAgentsAgentProfile.llm_profile_ref`` is a soft FK onto an LLM-profile
...
:class:`~configuration.profiles.agent_profile_store.AgentProfileStoreProtocol`,
...
from configuration.profiles.agent_profile_store import PROFILE_NAME_REGEX


if TYPE_CHECKING:
    from models.llm.llm_profile_store import LLMProfileMutator
    from configuration.profiles.agent_profile_store import AgentProfileStoreProtocol
...
def _scan_referrers(
    ...
    instantiation). Only the HRAgents variant carries ``llm_profile_ref``.
    """
    return [
        str(summary["name"])
        for summary in store.list_summaries()
        if summary.get("agent_kind", "HRAgent") == "HRAgent"
