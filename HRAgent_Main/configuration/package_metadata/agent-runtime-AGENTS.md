...
When adding non-Python files (JS, templates, etc.) loaded at runtime, add them to `HRAgent-agent-server/HRAgent/agent_server/agent-server.spec` using `collect_data_files`.
...
(under [#3117](https://github.com/HRAgents/software-agent-sdk/issues/3117)).
...
3. Do **not** use `HRAgent.sdk.utils.deprecation.deprecated` for FastAPI routes.
...
schema against the previous `HRAgent-agent-server` release selected from PyPI,
...
- FastAPI route handlers must not use `HRAgent.sdk.utils.deprecation.deprecated`.
