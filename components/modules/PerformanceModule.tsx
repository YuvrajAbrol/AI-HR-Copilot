...
// 9-box cell metadata. Index by box number (1..9).
const BOX_META: Record<number, { label: string; tone: string }> = {
  1: { label: "Risk", tone: "bg-rose-50" },
  2: { label: "Effective", tone: "bg-amber-50" },
  3: { label: "Trusted Pro", tone: "bg-amber-50" },
  4: { label: "Dilemma", tone: "bg-amber-50" },
  5: { label: "Core Player", tone: "bg-zinc-50" },
  6: { label: "High Impact", tone: "bg-emerald-50" },
  7: { label: "Enigma", tone: "bg-amber-50" },
  8: { label: "Growth", tone: "bg-emerald-50" },
  9: { label: "Star", tone: "bg-emerald-100" },
};
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {GRID_ROWS.flat().map((box) => {
          ...
          return (
            <div key={box} className={`min-h-[128px] rounded-lg border border-zinc-200 p-2 ${meta.tone}`}>
              ...
              <div className="flex flex-wrap gap-1">
                {people.slice(0, 10).map((e) => (
                  <span key={e.id} title={`${e.name} · ${e.performance.rating}/5`}>
                    <Avatar
                      initials={e.initials}
                      seed={e.name}
                      size="xs"
